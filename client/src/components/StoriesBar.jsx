...
const StoriesBar = () => {
  ...
  return (
    <div className="w-screen sm:w-[calc(100vw-240px)] lg:max-w-2xl no-scrollbar overflow-x-auto px-4">
      {/* If StoryViewer is active → show only that */}
      {viewStory ? (
        <StoryViewer viewStory={viewStory} setViewStory={setViewStory} />
      ) : showModal ? (
        // If Add Story modal is active → show only that
        <StoryModel setShowModal={setShowModal} fetchStories={fetchStories} />
      ) : (
        // Otherwise show the story bar UI
        <div className="flex gap-4 pb-5">
          ...
          {stories.map((story, index) => (
            <div
              key={index}
              onClick={() => {
                setShowModal(false); // ✅ close Add Story if open
                setViewStory(story); // ✅ open Story Viewer
              }}
